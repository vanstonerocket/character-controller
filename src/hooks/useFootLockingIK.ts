...
// - Trying several common castRay signatures to apply raycastGroups + excludeRigidBody.
...
type FootLockingOptions = {
    ...
    // Packed number (like collisionGroups) or interactionGroups(...) output.
    raycastGroups?: any;

    // Exclude the character rigidbody so rays do not immediately hit the capsule.
...
function findLegRig(scene: Object3D, side: "L" | "R", debugLog?: boolean): LegRig | null {
    ...
    // On RPM/Mixamo, ankle bone is usually the foot bone.
    const foot = ankle;
...
function castRayAttempts(
    ...
    const hasGroups = groups !== undefined && groups !== null;
    const hasExclude = excludeRigidBody !== undefined && excludeRigidBody !== null;

    // This matches how you already raycast in CharacterController:
    // castRay(ray, maxToi, solid, filterFlags, filterGroups, filterExcludeCollider, filterExcludeRigidBody)
    if (hasGroups || hasExclude) {
        tryCall("A: flags, groups, excludeRB (7 args)", () =>
            world.castRay(ray, maxToi, solid, undefined, hasGroups ? groups : undefined, undefined, hasExclude ? excludeRigidBody : undefined)
        );
    }

    // Some bindings: castRay(ray, maxToi, solid, filterGroups, ..., ..., excludeRB)
    if (hasGroups || hasExclude) {
        tryCall("B: groups 4th, excludeRB 7th", () =>
            world.castRay(ray, maxToi, solid, hasGroups ? groups : undefined, undefined, undefined, hasExclude ? excludeRigidBody : undefined)
        );
    }

    // Some bindings accept only 4 args: (ray, maxToi, solid, filterGroups)
    if (hasGroups) {
        tryCall("C: groups 4th only (4 args)", () => world.castRay(ray, maxToi, solid, groups));
    }

    // No groups but exclude RB in the 7th slot (useful if groups packing is wrong)
    if (!hasGroups && hasExclude) {
        tryCall("D: excludeRB 7th, no groups", () =>
            world.castRay(ray, maxToi, solid, undefined, undefined, undefined, excludeRigidBody)
        );
    }

    // Absolute fallback, no filters at all (can self-hit)
    tryCall("Z: no filters (3 args)", () => world.castRay(ray, maxToi, solid));
...
function twoBoneIKSolveWorld(
    ...
    const desiredKneeWorld = V3_D.copy(hipPos)
        .addScaledVector(xAxis, xk)
        .addScaledVector(yAxis, hk);
...
export function useFootLockingIK(
    ...
    const options = useMemo<Required<FootLockingOptions>>(() => {
        return {
            ...
            excludeRigidBody: opts?.excludeRigidBody ?? undefined,
    ...
    useEffect(() => {
        ...
        if (options.debugLog) {
            ...
            console.log("[useFootLockingIK] Initialized", {
                ...
                excludeRigidBody: options.excludeRigidBody ? "set" : "unset",
                note: "If you see hitY==originY, that is a self-hit (toi~0). We skip toi<=minToi and retry from higher origin.",
            });
        }
    }, [avatarScene, options.debugLog, options.raycastGroups, options.excludeRigidBody, options.minToi]);
        ...
        const solveFoot = (leg: LegRig, state: FootState) => {
            ...
            if (!options.debugFlatGround) {
                const castFromOrigin = (origin: Vector3) => {
                    ...
                    const attempts = castRayAttempts(
                        world,
                        ray,
                        options.rayLen,
                        true,
                        options.raycastGroups,
            ...
            if (
                ...
                console.warn(`[useFootLockingIK] ${leg.side} hit ABOVE ankle.`, {
                    ...
                    usedSig,
                    hint: "Usually self-hit. Ensure you pass excludeRigidBody and (optionally) set raycastGroups to floor only (0x00010004).",
                });
            }
